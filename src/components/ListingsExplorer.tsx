...
function ListingModal({ listing, onClose }: { listing: Listing; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[70]">
      ...
      <div className="absolute left-1/2 top-1/2 w-[min(1100px,92vw)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-[24px] border border-[color:var(--ui3)] bg-white shadow-xl">
        <div className="flex items-center justify-between gap-4 border-b border-[color:var(--ui3)] px-5 py-4">
          <div className="min-w-0">
            <div className="font-display truncate text-[20px] tracking-tight text-[color:var(--text)]">{listing.title}</div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={onClose}
              className="u-motion inline-flex h-10 w-10 items-center justify-center rounded-full border border-[color:var(--ui3)] bg-[color:var(--ui2)] text-[color:var(--text)] hover:bg-white"
          ...
          <div className="grid gap-8 p-5 lg:grid-cols-12">
            <div className="lg:col-span-7">
              ...
              <div className="mt-4 grid grid-cols-2 gap-4">
                <div className="relative h-[200px] overflow-hidden rounded-[var(--radius)]">
  ...
  return (
    <div className={className}>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {items.map((l) => (
          <ListingSummaryCard key={l.slug} listing={l} active={l.slug === selectedSlug} onClick={() => setSelectedSlug(l.slug)} />
        ))}
      </div>

      {selected ? <ListingModal listing={selected} onClose={() => setSelectedSlug(null)} /> : null}
    </div>
  );
}
